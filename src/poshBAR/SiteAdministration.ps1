...
function New-Site{
    ...
    if (!$exists) {
        ...
        Exec { "$appcmd add site /name:$siteName /physicalPath:$sitePath /bindings:$($bindingString -join ",")"} -retry 10 | Out-Null
        Exec { "$appcmd set app $siteName/ /applicationPool:$appPoolName"} -retry 10 | Out-Null
...
function Update-Site{
    ...
    if ($exists){
        ...
        Exec { "$appcmd set Site $siteName/ /bindings:$($bindingString -join ",")"} -retry 10  | Out-Null
        Exec { "$appcmd set App $siteName/ /applicationPool:$appPoolName"} -retry 10 | Out-Null
        Exec { "$appcmd set App $siteName/ `"/[path='/'].physicalPath:$sitePath`""} -retry 10 | Out-Null
        Write-Host "`tDone" -f Green
    }else{
